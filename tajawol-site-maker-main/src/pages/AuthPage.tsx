...
export default function AuthPage() {
    ...
    try {
      ...
      const redirectUrl =
        window.location.hostname === "localhost"
          ...
          : "https://tawsil-biougra.vercel.app/auth/callback";
      ...
      if (authData.user) {
        ...
        if (!profileData) {
          ...
          if (createProfileError) {
            ...
            // Still allow login but notify of profile creation error
            toast({
              variant: "destructive",
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Create the profile record if user was created
      if (authData.user) {
        ...
        // If no user was created but also no error, it's probably because email confirmation is required
        toast({
            ...
            <Card className="p-6 shadow-lg border-0 bg-card/95 backdrop-blur">
              <div className="space-y-6">
                <Button
                  type="button"
                  variant="outline"
                  className="w-full h-11 flex items-center justify-center gap-3 text-base hover:bg-accent/80 transition-colors"
